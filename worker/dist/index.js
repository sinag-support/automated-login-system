    ...
    try {
        ...
        for (let i = 0; i < accounts.length; i++) {
            ...
            try {
                ...
                if (i > 0) {
                    await automation_1.LoginAutomation.delay(2000, 5000);
                }
                // Attempt login
                ...
                // Create log entry
                await (0, db_1.createLoginLog)(account.id, result.success ? 'success' : 'failed', result.error);
                if (result.success) {
                    successCount++;
                }
                else {
                ...
                await (0, db_1.updateAccountStatus)(account.id, 'failed');
                await (0, db_1.createLoginLog)(account.id, 'failed', error.message);
                failCount++;
            }
        }
